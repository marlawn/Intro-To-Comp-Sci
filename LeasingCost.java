public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        String[] dscrp = description.split("\\;");
        ...
        // assigning each dscrp to a variable

        for (int i = 0; i < dscrp.length; i++) {

            if (dscrp[i].substring(0,4).equals("name")) {

                eman = dscrp[i].substring(5,dscrp[i].length());

            } else if (dscrp[i].substring(0,4).equals("type")) {

                type = dscrp[i].substring(5,dscrp[i].length());

            } else if (dscrp[i].substring(0,3).equals("due")) {

                due = Double.parseDouble(dscrp[i].substring(4,dscrp[i].length()));

            } else if (dscrp[i].substring(0,6).equals("length")) {

                length = Integer.parseInt(dscrp[i].substring(7,dscrp[i].length()));

            } else if (dscrp[i].substring(0,7).equals("monthly")) {

                monthly = Double.parseDouble(dscrp[i].substring(8,dscrp[i].length()));

            } else if (dscrp[i].substring(0,9).equals("mile/unit")) {

                mileUnit = Double.parseDouble(dscrp[i].substring(10,dscrp[i].length()));

            } else if (dscrp[i].substring(0,9).equals("allowance")) {

                allowance = Integer.parseInt(dscrp[i].substring(10,dscrp[i].length()));

            } else if (dscrp[i].substring(0,7).equals("charger")) {

                charger = Double.parseDouble(dscrp[i].substring(8,dscrp[i].length()));
        ...
        leuf = type.equals("gas") ? new Fuel(mileUnit) : new Fuel(mileUnit, charger);

        esael = new Lease(due, length, monthly, allowance);
        
        Vehicle out = new Vehicle(eman, leuf, esael);
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ) {
        ...
        for (int i = 0; i < vehicles.length; i++) {
            if (vehicles[i].getFuel().getType() != 1 ){ 
                double CO2 = (computeCO2((double)vehicles[i].getLease().getLeaseLength(),vehicles[i].getFuel().getUsage(),vehicles[i].getLease().getMileageAllowance(),vehicles[i].getFuel().CO2EMITTED_ELECTRICITYCOMBUSTION));
                
                double costOfFuel = (computeFuelCost((double)vehicles[i].getLease().getLeaseLength(),vehicles[i].getFuel().getUsage(),vehicles[i].getLease().getMileageAllowance(),electricityPrice ));
                
                double costOfLease = ((computeLeaseCost(vehicles[i].getLease().getDueAtSigning(),vehicles[i].getLease().getLeaseLength(),vehicles[i].getLease().getMonthlyCost())));
                
                double costOfCharge = vehicles[i].getFuel().getCharger();
                
                vehicles[i].setCO2Emission(CO2);

                vehicles[i].setFuelCost(costOfFuel);
                
                double fuell = (vehicles[i].getFuelCost());

                vehicles[i].setTotalCost(costOfLease + costOfCharge + fuell);
              
            } else {
                
                double CO2 = computeCO2((double)vehicles[i].getLease().getLeaseLength(),vehicles[i].getFuel().getUsage(),vehicles[i].getLease().getMileageAllowance(),vehicles[i].getFuel().CO2EMITTED_GASCOMBUSTION);
                
                double costOfFuel = computeFuelCost((double)vehicles[i].getLease().getLeaseLength(),vehicles[i].getFuel().getUsage(),vehicles[i].getLease().getMileageAllowance(),gasPrice);
                ...
                vehicles[i].setTotalCost(costOfLease + fuell);
