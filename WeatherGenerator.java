...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        if (random < 0.5) {
            forecast[0] = WET;                    // decides whether first day is wet or not
        } else {
            forecast[0] = DRY;
        }
        ...
        for (int i = 1; i < numberOfDays; i++) {
            ...
            if (forecast[i-1] == WET && random < wetwetProbability) {
                forecast[i] = WET;
            } else if (forecast[i-1] == DRY && random < drywetProbability) {
                forecast[i] = WET;
            } else {
                forecast[i] = DRY;
            }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        if (mode == DRY) {
            // determines whether all the values in the array are the same or not
            // also eliminates the possibility of the value of high being 0
            for (int i = 1; i < forecast.length; i++) {
                if (forecast[i-1] != forecast[i]) {
                    same++;
                } else {
                    if (forecast[i] == DRY) {
                        high = forecast.length;
                    } else {
                        high = 0;
                    }
                }
            ...
            if (same != 0) {
                high = 0;
                for (int i = 1; i < forecast.length; i++) {
                    if (forecast[i-1] == DRY && forecast[i] == DRY) {
                        streak++;
                    } else {
                        if (streak >= high) {
                            high = streak;
        ...
        if (mode == WET) {
            ...
            if (same != 0) {
                high = 0;
                for (int i = 1; i < forecast.length; i++) {
                    if (forecast[i-1] == WET && forecast[i] == WET) {
                        streak++;
                    } else {
                        if (streak >= high) {
                            high = streak;
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        int bestWeek = 0;
        int theWeek = 0;
        int[] bestWeekArray = new int[forecast.length - 6];
        int countOfDry = 0;
        ...
        for (int i = 0; i < (forecast.length - 6); i++) {
            ...
            bestWeekArray[i] = countOfDry;
            countOfDry = 0;
        }

        // FIX THIS WTF

        for (int i = 0; i < bestWeekArray.length; i++) {
            if (bestWeekArray[i] > bestWeek) {
                bestWeek = bestWeekArray[i];
                theWeek = i;
            }
        }
    ...
    public static void main (String[] args) {
        ...
        StdRandom.setSeed(1647380605377L);
